...
;; Note: :vat typo validation lives in `invoices.calc/validate-item-vat!`
;; and runs from `set-price` before items reach this namespace. See
;; `calc_test.clj` for the discriminator tests. `fa/invoice->fa3-xml`
;; itself trusts that its input has already cleared that gate, so a
;; direct caller passing `:vat :np-us` would silently route to zw —
;; which is why the validation was hoisted upstream in the first place.
